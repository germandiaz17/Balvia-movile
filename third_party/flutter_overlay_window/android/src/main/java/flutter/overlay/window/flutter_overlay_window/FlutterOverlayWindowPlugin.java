...
public class FlutterOverlayWindowPlugin implements
    ...
    // PATCH: channel of the FIRST engine to attach — the main app's. Upstream
    // kept only WindowSetup.messenger, which the overlay engine overwrites on
    // attach (it registers last), so messages sent FROM the overlay were
    // forwarded straight back to the overlay itself and the main app never
    // received them (breaking "expense saved in bubble → app pushes sync").
    // OverlayService forwards overlay-originated messages to this instead.
    static BasicMessageChannel<Object> mainMessenger = null;
    ...
    public void onAttachedToActivity(@NonNull ActivityPluginBinding binding) {
        ...
        if (FlutterEngineCache.getInstance().get(OverlayConstants.CACHED_TAG) == null) {
            ...
            // FIX: the 2-arg DartEntrypoint(bundlePath, functionName) constructor relies on
            // Dart_RootLibrary to resolve "overlayMain" by name across the whole compiled
            // program, which is broken for secondary (non-main) entrypoints in release/AOT
            // builds (flutter/flutter#91841 — a long-standing, still-unfixed Flutter engine
            // bug affecting FlutterEngineGroup-based plugins). The documented workaround
            // (flutter/engine#30726) is to specify the entrypoint's library URI explicitly.
            DartExecutor.DartEntrypoint dEntry = new DartExecutor.DartEntrypoint(
                    FlutterInjector.instance().flutterLoader().findAppBundlePath(),
                    "package:balvia_mobile/features/overlay/overlay_entry.dart",
                    "overlayMain");
