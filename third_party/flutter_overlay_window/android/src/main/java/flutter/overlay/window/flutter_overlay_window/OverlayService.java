...
public class OverlayService extends Service implements View.OnTouchListener {
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        flutterView = new FlutterView(getApplicationContext(), new FlutterSurfaceView(getApplicationContext(), true));
        ...
        overlayMessageChannel.setMessageHandler((message, reply) -> {
            // PATCH: forward overlay-originated messages to the MAIN app's
            // channel. WindowSetup.messenger points at the overlay engine
            // (last to attach), which would bounce the message back to its
            // sender and the app would never hear it.
            if (FlutterOverlayWindowPlugin.mainMessenger != null) {
                FlutterOverlayWindowPlugin.mainMessenger.send(message);
            }
        });
        ...
        // PATCH: upstream passed WindowSetup.width/height straight to
        // LayoutParams as raw pixels, while resizeOverlay() converts dp→px.
        // Convert here too so showOverlay and resizeOverlay agree on dp —
        // otherwise the initial window is ~2.6x too small on this density and
        // positions computed against it break after the first resize.
        ...
        // PATCH: never let the IME pan/resize this overlay window. The expanded
        // capture card is placed in the upper half of the screen by the app, so
        // the keyboard never overlaps it; without this the window jumps when the
        // IME animates in.
        params.softInputMode = WindowManager.LayoutParams.SOFT_INPUT_ADJUST_NOTHING;
        flutterView.setOnTouchListener(this);
        windowManager.addView(flutterView, params);
        // PATCH: upstream calls moveOverlay() synchronously right after addView(),
        // before the view's first layout/draw pass completes. On newer Android
        // (BLASTBufferQueue) that races with the initial surface setup. Defer to
        // the next frame via View.post() so the first layout finishes first.
        flutterView.post(() -> moveOverlay(dx, dy, null));
        return START_STICKY;
    }
    ...
    // PATCH: stop any in-flight edge-snap animation before a programmatic resize
    // or move; its 25ms timer keeps calling updateViewLayout until convergence and
    // would overwrite the position we are about to set.
        ...
        if (windowManager != null && WindowSetup.enableDrag) {
            ...
            switch (event.getAction()) {
                ...
                case MotionEvent.ACTION_CANCEL:
                    ...
                    // PATCH: only run the edge-snap animation after an actual drag.
                    // Upstream schedules it on every ACTION_UP, so a plain tap kicked
                    // off a ~300ms animation that fought programmatic resize/move
                    // (e.g. expanding the capture card) and left the window misplaced.
